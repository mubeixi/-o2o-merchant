		...
		:on-change='change'>
			...
			<div slot="file" slot-scope="{file}">
				<template v-if="file.status!='success'">
					<el-progress :width="width-2" style="width: 100%" type="circle"
					             :percentage="file.percentage|percent"></el-progress>
				</template>
				<template v-if="file.status=='success'">
					<video
					v-if="type==='video'"
			...
			<!--      </template>-->
			<div slot="tip" v-if="tip" class="el-upload__tip ">{{tip}} <i @click="remove"
			                                                              style="position: absolute;right: 0;top: 0;font-size: 22px;cursor: pointer;"
			                                                              v-if="showDelIcon && imgUrl"
			                                                              class="el-icon-circle-close del-icon"></i></div>
		</el-upload>
