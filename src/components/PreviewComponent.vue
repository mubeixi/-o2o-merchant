<template>
	<div class="preview-body" id="previewWrap" @click="bindWrapClick">
		<!--default也可以显示顶部-->
		<div class="preview-head-new" v-if="!isDiy && system.topTheme==='default'">
			<div class="preview-header-title"></div>
		</div>
		<div class="preview-head" v-if="isDiy || system.topTheme!=='default'">
		...
		<div class="canvasBox" @drop="dropEv" @dragover.prevent>
			...
			<div class="canvas" id="canvas" :style="{backgroundColor:system.bgcolor}"
				...
				<div v-if="!isDiy  && system.topTheme==='default'" class="kill noselect" @click.stop="noop">
				...
				<div v-if="!isDiy && system.topTheme==='default'" class="live noselect" @click.stop="noop">
					...
					<div class="live-content">
						<div class="live-list">
							<div class="live-item" v-for="(item,idx) in goodsList" :key="idx">
								...
								<div class="right">
									<div class="price-box">
										<div class="selling-price font12">
											<span class="sign">￥</span><span class="num">{{item.Products_PriceX}}</span>
											<span class="sign"
											      style="color: #999;text-decoration: line-through;margin-left: 5px">￥</span><span
										style="color: #999;text-decoration: line-through;" class="num">{{item.Products_PriceY}}</span>
										</div>
									</div>
									<div class="sale-count">已抢{{item.Products_Sales}}份</div>
									<!--                  <div class="tags" v-if="item.tags">-->
									<!--                    <span class="tag" v-for="(tag,idx) in item.tags" :key="idx">{{tag}}</span>-->
									<!--                  </div>-->
									<div class="tags">
										<span class="tag">营养</span>
				...
				<div class="removeModal" ref="removeModal" v-show="removePosition.show">
					...
					<h4>您确定要删除这个组件吗？</h4>
					<p>
						<input type="button" value="确定" class="ok" @click.stop="removeTemplate" />
						<input type="button" value="取消" class="on" @click.stop="removePosition.show = false" />
					</p>
				</div>
				<ul
...
import {GetQueryByString, getStyle, objTranslate} from '@/common/utils';
...
@Component({
    ...
    bindWrapClick(){
      this.$refs.rightMenu.style.display = 'none'; //隐藏删除的菜单
    },
	...
	async created() {
		...
		new Promise((resolve, reject) => {
			...
			.then(mixinData => {
				...
				if (mixinData.system) {
					this.setSystem(Object.assign({topTheme:'none'},mixinData.system));
				}
		...
		const goodsDatares = await getProductList({pageSize: 6}).catch((e) => {
			throw new Error(e.msg)
		})
		this.goodsList = goodsDatares.data

		const cateDataRes = await getProductCategory({pageSize: 20}).catch((e) => {
			throw new Error(e.msg)
		})
		this.cates = cateDataRes.data
		...
		.nav-box {
			...
			.nav-list {
				...
				.nav-item {
					...
					&.active {
						color: #26C78D;
						border-bottom: 2px solid #26C78D;
					}
				}
			}
		}
