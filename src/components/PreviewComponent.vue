                ...
                <div class="right">
                  <div class="price-box">
                    <div class="selling-price font12">
                      <span class="sign">￥</span><span class="num">{{item.Products_PriceX}}</span>
                      <span class="sign" style="color: #999;text-decoration: line-through;margin-left: 5px">￥</span><span style="color: #999;text-decoration: line-through;" class="num">{{item.Products_PriceY}}</span>
                    </div>
                  </div>
...
import {
  ...
  setDiySkinConfig,
  getProductList,
  bizProdCateList,getProductCategory
} from '@/common/fetch';
...
export default class PreviewComponent extends Vue {
  ...
  liveList = []
    ...
    async created() {
      ...
      const goodsDatares = await getProductList({pageSize:6}).catch((e)=>{throw new Error(e.msg)})
      this.goodsList = goodsDatares.data

      const cateDataRes = await getProductCategory({pageSize:20}).catch((e)=>{throw new Error(e.msg)})
