      ...
      :key="index">
        ...
        :imgUrl='item.model'></upload-img-components>
				...
				<div v-if="item.type==='arr' && item.value.length>0" class="arr-box">
					<div v-for="(arr_item,idx) in item.value" :key="idx" class="arr-row" style="">
						<!--简单值-->
						...
						<div class="row-container" v-if="['cate'].indexOf(item.row_type)!==-1">
							...
							<div class="flex line10">
								...
								<div>
									<el-input size="small" autosize v-model="item.value[idx].title" class="input" @input='item.inputCB' />
								</div>
							</div>
							<div class="flex">
				...
				<div v-if="item.type === 'diy'" class="">
					...
					<template v-if="item.row_type === 'buybtn'">
						...
						<div style="display: inline-block;">
							<!--             padding-top: 14px-->
							<el-radio-group style="margin-left: 20px;" v-model="item.data.style" @change="item.radioCB">
              ...
              :imgUrl='item.data.img'></upload-img-components>
				...
				<div v-if="item.type === 'origin' && !item.flashsale_flag">
					...
					<div v-if="item.model==='filter'" style="margin-left: -70px;margin-top: 8px">
						<el-tooltip class="item rightBtn" effect="dark" :content="item.origintooltip" placement="right">
							<el-button
							@click="openGoodsBindList(item,item.bindListCB,item.pintuan_flag,item.flashsale_flag,item.kill_flag,item.has)"
							type="primary"
							size="small">选择商品
							</el-button>
						</el-tooltip>
				...
				<div v-if="item.type === 'origin' && item.flashsale_flag">
					<el-tooltip class="item rightBtn" effect="dark" :content="item.origintooltip" placement="right">
						<el-button @click="openSpikeBind(item,item.bindItemCB,item.spike_id)" type="primary" size="small">绑定活动
						</el-button>
					</el-tooltip>
				</div>
