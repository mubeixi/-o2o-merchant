			...
			<div class="container">
				<div class="container-left">
					<div class="container-leftTop">
						<template v-for="(dir,idx) in dirs">
							<div class="item js-type-item" v-if="filterSourceType(dir.source_type)"
							     :class="{active:idx==current_type_idx}" @click="selectSourceType(dir,idx)">
								{{dir.label}}
							</div>
						</template>
				...
				<div class="container-right" v-loading="reqLoading">
					...
					<div class="container-right-image" style="height:440px;overflow: hidden">
						...
						<div class="image" :class="{check:file.checked && select_file_list.length>0}" :key="idx2"
						     v-for="(file,idx2) in current_file_list">
							<!--文件夹先显示-->
							...
							<template v-else>
								<div style="position: absolute;width: 100%;height: 100%;">
									...
									<template v-if="source_type=='image'">
										<div @click="addFn(file)" class="img-cover"
										     v-lazy:background-image="getFileUrl(file.fileurl)"></div>
									</template>
									<div class="file_name">
										{{file.filename}}
									</div>
					...
					<div class="paginate-box">
						<div style="text-align: right;">
							<el-button @click="minusPage" size="mini"><</el-button>
							<div style="display: inline-block" class="padding10-c">{{currentPage}}/{{totalPage}}</div>
							<el-button size="mini" @click="plusPage">></el-button>
...
import {createDirectory, getFileList} from '../../common/fetch'
...
export default class WzwFinder extends Vue {
  ...
  dirs = [
    ...
    {label: '视频', source_type: 'media', accept: 'audio/mp4, video/mp4'},//,application/ogg, audio/ogg,.wmv
    {
      label: '文件',
      source_type: 'file',
      accept: 'application/vnd.ms-powerpoint,application/vnd.ms-excel,application/msword,application/pdf'
    },
    {
      label: '其他',
      source_type: 'other',
