...
@Component({
  ...
  computed: {
    computedColumns() {
      const list = this.columns.filter(column => column.showIf ? column.showIf(this.lists,column) : true)

	    // //默认为false
	    // const result = list.map(column=>{
	    //   if(!column.hasOwnProperty('selectableFn')){
      //     column.selectableFn = ()=>true
	    //   }
	    //
      //   return column
	    // })
...
export default class FunTable extends Vue {
  ...
  handleRowChange(row, column, event) {
    ...
    if (this.isRow) return
	  // 如果是只允许增加的情况,而且当前行已经被选中，那么就返回
	  if(this.isOnlyAdd && valInArr(row[this.vkey], this.has)){
	    console.log(row[this.vkey],this.has)
	    return
	  }
