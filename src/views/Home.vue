<template>
  <div class="home-wrap">
    <div class="flex main">
      ...
      <div class="preview" id="preview">
        <div class="preview-page">
          <div :class="{active:mode==='system'}" @click="setAct(0,'system','全局设置','针对页面的整体配置')"
               class="item preview-page-options"><i class="preview-page-options-icon"></i>全局设置
          </div>
          <div :class="{active:mode==='plugin'}" @click="setAct(1,'plugin','组件管理','可以便捷拖动、删除组件')"
               class="item preview-page-coms"><i class="preview-page-coms-icon"></i>组件管理
          </div>
        </div>
        <preview-component @preFun="setPreEv" @setData="setDataEv"
                           ref="preview"></preview-component>
      </div>
      <div class="setattr">
        <div class="deco-component-title">
          <div class="deco-component-title__header"><span class="deco-component-title__name">{{componentTitle.title}}</span>
          </div>
    ...
    <div class="handle">
      <el-button @click="saveData(0)" size="small" type="primary">保存</el-button>
      <el-button @click="clearPlugin" size="small" type="danger" v-if="is_dev">清空组件</el-button>
