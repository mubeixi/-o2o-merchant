			...
			<div class="lists">
              ...
              class="status-text danger-color padding10-c font14">{{apply.Order_Status_desc}}</span>
                    ...
                    <span class="handle" v-if="inArray(apply.Order_Status,[20,22,25])">
                      ...
                      <input :value="item.prod_count" @blur="setValFn($event,apply,item,idx1)" class="input" min="1"
                             type="number" />
                      <span @click="plusFn(apply,item,idx1)" class="plus">
...
export default class StorePurchaseApply extends Vue {
  ...
  async changeApplyChannel() {
    ...
    await getStoreDetail({
      store_sn: this.channelDialogInstance.store_no,
      User_ID: null,
      store_id: null
    }).then(res => {
      rt = true
