        ...
        <div :key="idx1" class="item" v-for="(apply,idx1) in applys">
          <div class="head flex">
            <div class="info flex flex1">
              <div :style="{backgroundImage:'url('+apply.receive_img+')'}" class="store-pic"></div>
            ...
            <div class="status">
              ...
              <i @click="delApply(apply,idx1)" class="el-icon-delete-solid"
                 title="删除订单" v-if="inArray(apply.Order_Status,[21,23,25])"></i>
            </div>
          </div>
          <table cellspacing="0"
                 class="purchases"
            ...
            <tr :key="idx2" class="goods-list" v-for="(item,idx2) in apply.prod_list">
              <td class="goods">
                ...
                <div class="r font14"><span class="danger-color">￥<span class="price-num font18">{{item.prod_price}}</span></span>
                </div>
              </td>
              ...
              <td :rowspan="apply.prod_list.length" class="actions text-center" v-if="idx2===0">
                ...
                <div class="line10" v-if="inArray(apply.status,[35])">
                  ...
                  >
                    ...
                    <div style="text-align: right; margin: 0">
                      <el-button @click="apply.checkVisible = false" size="mini" type="text">取消
                      </el-button>
                      <el-button @click="payApplyFn(apply,idx1)" size="mini" type="primary">确定
                      </el-button>
                    </div>
                    <el-button @click.prevent="apply.checkVisible = true" class="acion-btn"
                ...
                <div class="line10" v-if="inArray(apply.status,[31])">
                  ...
                  >
                    ...
                    <div style="text-align: right; margin: 0">
                      ...
                      <el-button @click="cancelFn(apply,idx1)" size="mini" type="primary">确定
                      </el-button>
                    </div>
                ...
                <div class="line10" v-if="inArray(apply.status,[32])">
                  ...
                  >
                    <p>是否确认发货?</p>
                    <div style="text-align: right; margin: 0">
                      <el-button @click="apply.sendVisible = false" size="mini" type="text">取消
                      </el-button>
                      <el-button @click="openSendDialog(apply,idx1)" size="mini" type="primary">确定
                      </el-button>
                    </div>
                    <el-button @click.prevent="apply.sendVisible = true" class="acion-btn"
                               size="small" slot="reference"
                               type="primary">发货
                    </el-button>
                  </el-popover>
                </div>
                <div @click="showLogistics(apply)"
                     class="font12 graytext2 logistics"
                     v-if="apply.Order_ShippingID && inArray(apply.status,[34,35,36])">查看物流
                </div>
              </td>
            </tr>
          </table>
      ...
      <div class="dialog-container">
        <el-form class="form" label-width="120px">
          <el-form-item label="是否需要物流">
            <el-radio label="1" v-model="sendDialogInstance.is_need_shipping">需要</el-radio>
            <el-radio label="0" v-model="sendDialogInstance.is_need_shipping">不需要</el-radio>
          </el-form-item>
          <el-form-item label="物流公司" v-show="sendDialogInstance.is_need_shipping==1">
            <el-select v-loading="sendDialogInstance.loading" v-model="sendDialogInstance.express">
              <el-option
                :key="item"
                :label="item"
                :value="item"
          ...
          <el-form-item label="订单单号"
                        v-show="sendDialogInstance.is_need_shipping==1 && sendDialogInstance.express">
            <el-input placeholder="请输入单号" v-model="sendDialogInstance.shipping_no" />
          </el-form-item>
        </el-form>

        <div class="padding4-c"></div>
        <div style="text-align: right">
          <el-button :loading="sendDialogInstance.send" @click="sendFn" type="primary">发货
          </el-button>
        </div>
...
export default class StorePurchaseApply extends Vue {
  ...
  async sendFn() {
    ...
    let postData = {
      ...
      express,
      is_need_shipping,
      order_id: this.sendDialogInstance.apply.id
    }
  ...
  showLogistics(apply) {
    ...
    let {out_order_no = '', Express = ''} = {
      ...apply.Order_Shipping,
      out_order_no: apply.Order_ShippingID
    }
